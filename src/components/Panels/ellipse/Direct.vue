<template>
  <Layout :panel_show.sync="panel_show" title="椭圆绘制-直接绘制">
...
function computeRotate(arr) {
  const p1 = Cesium.Cartographic.fromCartesian(arr[0]);
  const p2 = Cesium.Cartographic.fromCartesian(arr[1]);
...
export default {
  ...
  computed: {
    ...
    maxHeight() {
      return parseFloat(innerHeight) - 438;
    }
  },
  watch: {
    curSelect(val) {
      val === "draw-ellipse-direct" && (this.panel_show = true);
    }
  },
  beforeDestroy() {
    this.clear();

<template>
  <Layout :panel_show.sync="panel_show" title="分队战斗行动绘制-直接绘制">
...
// * 获取分队战斗行动坐标
function getSquadCombat(pnts) {
  const tailPnts = getTailPoints(pnts);
...
function create() {
  ...
  const res = getSquadCombat(pointList);
...
export default {
  ...
  watch: {
    curSelect(val) {
      val === "draw-squadCombat-direct" && (this.panel_show = true);
    }
  },
  beforeDestroy() {
    this.clear();

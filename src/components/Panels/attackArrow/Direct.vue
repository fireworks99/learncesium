<template>
  <Layout :panel_show.sync="panel_show" title="进攻方向绘制-直接绘制">
...
function create() {
  ...
  const res = getAttackArrow(pointList);
...
export default {
  ...
  watch: {
    curSelect(val) {
      val === "draw-attackArrow-direct" && (this.panel_show = true);
    }
  },
  beforeDestroy() {
    this.clear();

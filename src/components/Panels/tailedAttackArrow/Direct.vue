<template>
  <Layout :panel_show.sync="panel_show" title="进攻方向（尾）绘制-直接绘制">
...
// * 获取进攻方向尾坐标
function getTailedAttackArrow(pnts) {
  let tailLeft = pnts[0];
...
function create() {
  ...
  const res = getTailedAttackArrow(pointList);
...
export default {
  ...
  watch: {
    curSelect(val) {
      val === "draw-tailedAttackArrow-direct" && (this.panel_show = true);
    }
  },
  beforeDestroy() {
    this.clear();
